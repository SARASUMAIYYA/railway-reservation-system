        ...
        if (state.checked === 'card') {
            if (state.cardNo && state.cvc && state.exp) {
                validateCard({ card: state.cardNo, cvc: state.cvc, exp: state.exp, total: state.total })
                    .then(res => {
                        if (res.validated) {
                            console.log("trial 1",res.validated);
                        ...
                        <Table striped bordered hover size="sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td align='right'>{this.state.amount} Rs</td>
                                </tr>
                                <tr>
                                    <td align='right'>Discount</td>
                                    <td align='right'>{this.state.discount} Rs</td>
                                </tr>
                                <tr>
                                    <td align='right'>Total</td>
                                    <td align='right'>{this.state.total} Rs</td>
                                </tr>
                            </tbody>
                        </Table>
                    </Form.Row>
                    ...
                    {this.state.checked === 'card' &&
                        <Form.Row style={{ width: '75%' }}>
                            <Form.Group as={Col} controlId="cardNo">
                                ...
                                <Form.Control placeholder="card number" type='number' onChange={this.handleChange('cardNo')} value={this.state.cardNo} />
                            </Form.Group>
                            <Form.Group as={Col} controlId="cvc">
                                <Form.Label>CVC Number</Form.Label>
                                <Form.Control placeholder="CVC" type='number' onChange={this.handleChange('cvc')} value={this.state.cvc} />
                            </Form.Group>
